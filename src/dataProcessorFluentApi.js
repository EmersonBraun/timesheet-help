export class DataProcessorFluentAPI {
    ...
    #formatDate(date) {
        ...
        const { groups: { day, month, year } } = regex.exec(date)
        return `${month}/${day}/${year}`
    }

    removeUnuseds() {
        const data = this.#data.Report
            .filter(({ A, B, C, D, E, F, G }) => {
                if (A !== 'Total' && B !== 'Access restricted' && B !== 'Fix version') {
                    return {
                        B, C, D, E, F, G
                    }
                }
        ...
        const data = [
            {
                ...
                columns: [
                    ...
                    { label: 'Customer (if applicable) ', value: 'Customer (if applicable) ' },
                ],
                content: this.#data
            }
        ]
